...
function verfizieren($email,$password){
  ...
  if(empty($data)){
    ...
    if($erfolganmelden){
      ...
      $anzahlfehler= "call anzahlfehler('$email') ";
      mysqli_query($link,$anzahlfehler);
...
function idnamelist(){
  try {
    $link = connectdb();
    //limit 30
    $sql = 'SELECT id,name FROM benutzer ORDER BY id limit 30';
...
function getallbenutzer(){
    ...
    $sql = 'SELECT * FROM benutzer ORDER BY id';
    $result = mysqli_query($link, $sql);
